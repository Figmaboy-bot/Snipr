...
// Loads SniprMerge (self.SniprMerge) — the same import/dedup logic used by
// the webapp, built from shared/merge.js so both sides stay in sync.
importScripts("shared/merge.bundle.js");
  ...
  (async () => {
    switch (message.type) {
      ...
      case "IMPORT_ALL": {
        ...
        const { newSaves, skippedSaves, newFolders, newCategories } =
          self.SniprMerge.computeImportDelta({ saves, folders, categories }, message.data || {});

        await chrome.storage.local.set({
          saves: [...saves, ...newSaves],
